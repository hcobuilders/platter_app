...
| Q-16 | `STEP1_PLAN.md`'s data model (Part A) has no lifecycle-cost table. A `lifecycle_line` shape (system, initial_cost, service_life_yrs, annual_maintenance, source_confidence) is the obvious fit for Batch 5's 5.4, but it's new schema surface, not yet reviewed. | Open — raised S11, doesn't block wireframing |
